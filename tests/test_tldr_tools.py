...
class TldrToolTest(unittest.TestCase):
    def test_scaffold_uses_immutable_attempt_evidence(self):
        ...
        new_tldr = load_module("new_tldr", REPO / "tools/new_tldr.py")
        text = new_tldr.render_session(result)
    ...
    def test_cli_generates_without_touching_repository_session(self):
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "0712-tldr.md"
            subprocess.run(
                [
                    "python3",
                    str(REPO / "tools/new_tldr.py"),
                    "--date",
                    "0712",
                    "--output",
                    str(output),
            ...
            self.assertIn("# 0712｜TLDR 草稿", output.read_text(encoding="utf-8"))
