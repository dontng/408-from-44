...
## 已完成
- **studio 刷题台**：`bash studio.sh` 一键上线本地网页（127.0.0.1:8408，免代理）。**和室·侧栏+题流**版式：左细栏（186 倒计时 / 进度圆点 / 点击跳题），右侧当天**全部题一条纵向流**（不再一题一屏），题面图裁白铺满、ABCD 小方块横排、答完原地判分。遗忘曲线状态存 `review/state.json`（可多机同步）。无官方答案的题自动转**自判模式**，不会误判。
