...
1. 生成今日题单：`./tod 0705`。不传日期时默认今天：`./tod`。
2. 启动答题卡：`./ans 0705`。
...
另外，Git 安全网会在每天 02:00 与 20:00 检查一次工作区；**仅当“已到该时点”且“存在未提交改动”同时成立**，才创建一个完整恢复点并推送。若项目仍在编辑或讨论，它会等到连续 10 分钟无活动再提交。它不替代正常的 handoff，只避免白天或睡前忘记同步而让工作只能留在一台机器上。
